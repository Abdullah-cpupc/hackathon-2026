...
export default function EditWidgetPage() {
  ...
  const form = useForm<WidgetForm>({
    ...
    defaultValues: { 
      ...
      minimizedBgColor: '#2563eb', 
      maximizedStyle: 'solid', 
      systemBubbleBgColor: '#f3f4f6', 
      userBubbleBgColor: '#2563eb' 
    },
  })
  ...
  useEffect(() => {
    const fetchWidget = async () => {
      try {
        const widgetData = await widgetAPI.getWidget(widgetId)
        setWidget(widgetData)
        
        // Set form values from fetched widget data
        form.reset({
          name: widgetData.name,
          position: widgetData.position,
          minimizedShape: widgetData.minimized_shape,
          minimizedBgColor: widgetData.minimized_bg_color,
          maximizedStyle: widgetData.maximized_style,
          systemBubbleBgColor: widgetData.system_bubble_bg_color,
          userBubbleBgColor: widgetData.user_bubble_bg_color,
        })
      } catch (error) {
        console.error('Failed to fetch widget:', error)
        // Redirect back to company page if widget not found
  ...
  }, [widgetId, companyId, router, form])
  ...
  const onSubmit = async (values: WidgetForm) => {
    ...
    try {
      ...
      await widgetAPI.updateWidget(widgetId, widgetData)
      
      // Redirect to company page with success message
      router.push(`/dashboard/companies/${companyId}?widgetUpdated=true`)
    } catch (error) {
  ...
  if (!widget) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-8 h-8 bg-gradient-to-r from-red-600 to-red-700 rounded-lg flex items-center justify-center mx-auto mb-4">
            <X className="w-5 h-5 text-white" />
          </div>
          <p className="text-gray-600">Widget not found</p>
          <Button asChild className="mt-4">
            <Link href={`/dashboard/companies/${companyId}`}>Back to Company</Link>
          </Button>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-stretch min-h-[calc(100vh-10rem)]">
          {/* Left: Configuration Form */}
          <div className="lg:col-span-1">
            <Card className="border-0 shadow-xl h-[86vh]">
              <CardHeader>
                <CardTitle className="text-2xl font-bold">Edit Chat Widget</CardTitle>
                <CardDescription>Update your website chat widget configuration.</CardDescription>
              </CardHeader>
